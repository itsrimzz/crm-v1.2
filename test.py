import os, io, csv
from flask import Flask, render_template, json, request, session, redirect, make_response
...
def getReport():
    #try:
        if session.get('user') == 'master':
            ...
            account_data = "hi"
            for account in data:
                #account_data+=str(account[0])+","+account[1]+","+account[2]+','+account[3]+','+account[4]+','+account[5]+','+account[6]+','+account[7]+','+account[8]+','+account[9]+','+account[10]+','+account[11]+','+account[12]+','+str(account[13])+','+account[14]+','+account[15]+','+account[16]+'\n'
                temp =str(account[0])+''',"'''+account[1]+'''"'''+''',"'''+account[2]+'''"'''+''',"'''+account[3]+'''"'''+''',"'''+account[4]+'''"'''+''',"'''+account[5]+'''"'''+''',"'''+account[6]+'''"'''+''',"'''+account[7]+'''"'''+''',"'''+account[8]+'''"'''+''',"'''+account[9]+'''"'''+''',"'''+account[10]+'''"'''+''',"'''+account[11]+'''"'''+''',"'''+account[12]+'''"'''+','+str(account[13])+''',"'''+account[14]+'''"'''+''',"'''+account[15]+'''"'''+''',"'''+account[16]+'''"\n'''
                #account_data+=temp
            return temp
            #os.system("mv /tmp/table.csv /home/rishabh/Desktop/crm/static/")
            #return redirect('/static/table.csv')
            """response = make_response(account_data)
...
def getCaseReport():
    try:
        if session.get('user') == 'master':
            ...
            data = cursor.fetchall()
            conn.commit()
            os.system("mv /tmp/case.csv /home/rishabh/Desktop/crm/static/")
